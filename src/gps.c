...
void gpsInit(uint8_t baudrateIndex)
{
    ...
    // if the port is in use , we stop
    if (serialPortMap[mcfg.gps_port] > 1)
        return;

    switch (mcfg.gps_port) {
        case SERIALPORT_SOFT_1:
        case SERIALPORT_SOFT_2:
            core.gpsport = &(softSerialPorts[mcfg.gps_port].port);
            break;
    ...
    serialPortMap[mcfg.gps_port] = serialPortMap[mcfg.gps_port] + 1;
