...
extern pid_param_t navPID_PARAM;
extern pid_param_t altPID_PARAM;
...
void fw_nav(void)
{
    ...
    if (millis() - gpsTimer >= gpsFreq) {
        ...
        if (abs(GPS_AltErr) < 1)
            NAV_Thro = cfg.cruise_throttle; // Just cruise along in deadpan.
        else {
            // Add AltitudeError  and scale up with a factor to throttle
            NAV_Thro = constrain(cfg.cruise_throttle - (GPS_AltErr * cfg.scaler_throttle), cfg.idle_throttle, cfg.climb_throttle);
        }
        ...
        ******************************/
        ...
        NavDT = (float) (millis() - nav_loopT) / 1000.0f;
        nav_loopT = millis();
        /****************************************************************************************************/
        // Altitude PID
